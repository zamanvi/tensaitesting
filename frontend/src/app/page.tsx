...
export default function HomePage() {
  ...
  const navAbout  = ja ? '私たちについて' : bn ? 'আমাদের সম্পর্কে' : 'About';
  const navTeam   = ja ? 'チーム'         : bn ? 'দল'               : 'Team';
  ...
  return (
    <div className="min-h-screen bg-[#0d1117]">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'glass-nav' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <Link
              href="/auth/register"
              className="text-sm bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-full font-semibold transition-all glow-green"
            >
              {l.getStarted}
            </Link>
          </div>
      ...
      <section className="hero-mesh min-h-screen flex items-center px-4 pt-20 pb-16 relative overflow-hidden">
        ...
        <div className="absolute top-[15%] left-[8%]  w-[480px] h-[480px] bg-green-600/12  rounded-full blur-[140px] pointer-events-none" />
        <div className="absolute bottom-[20%] right-[5%]  w-[380px] h-[380px] bg-cyan-500/8   rounded-full blur-[120px] pointer-events-none" />
        <div className="absolute top-[60%] left-[40%]  w-[300px] h-[300px] bg-violet-600/6  rounded-full blur-[100px] pointer-events-none" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full">
          <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-20">
            ...
            <div className="flex-1 text-center lg:text-left animate-fade-up">
              ...
              {/* Sub-label */}
              <p className={`text-white/28 text-xs font-medium mb-5 ${lang === 'en' ? 'tracking-[0.25em] uppercase' : 'tracking-normal'}`}>
              ...
              {/* CTAs — student dominant, agency+institution secondary */}
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center lg:justify-start gap-3 mb-10 max-w-lg lg:mx-0 mx-auto">
                <Link
                  ...
                  className="flex-[1.4] text-center bg-green-600 hover:bg-green-500 text-white px-6 py-4 rounded-full font-bold text-sm transition-all glow-green"
                >
                  {l.ctaStudent}
                </Link>
                <Link
                  href="/auth/register?type=agency"
                  className="flex-1 text-center glass-card text-white/75 hover:text-white px-5 py-3.5 rounded-full font-semibold text-sm transition-all border border-white/10 hover:border-white/22"
                >
                  {l.ctaAgency}
                </Link>
                <Link
              ...
              <div className="overflow-x-auto justify-center lg:justify-start flex">
                <div className="inline-flex items-stretch rounded-2xl overflow-hidden border border-white/[0.08] divide-x divide-white/[0.08] min-w-max">
                  {STATS.map((s) => (
                    <div key={s.label} className="px-5 sm:px-6 py-3 bg-white/[0.03] text-center">
            ...
            {/* ── Right: Student Profile Mockup ───────────── */}
            <div className="hidden lg:block lg:flex-none w-72 xl:w-80 animate-slide-right">
      ...
      {/* ── Trust Ticker ───────────────────────────────────── */}
      <div className="border-y border-white/[0.05] py-3.5 overflow-hidden bg-white/[0.015]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {GATEWAYS.map((g) => (
            <Link
              ...
              className={`group rounded-2xl p-6 flex flex-col gap-4 transition-all duration-300 relative overflow-hidden
                ${g.featured
      ...
      <section className="bg-alt-section py-20 border-t border-white/[0.05]">
        <div className="max-w-5xl mx-auto px-4">
            ...
            {HOW_IT_WORKS.map((step, i) => (
              <div key={step.num} className="flex flex-row md:flex-col items-start md:items-center md:flex-1 gap-5 md:gap-0 md:text-center">
                ...
                <div className="flex flex-col items-center gap-2 shrink-0">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-green-500/20 to-green-600/5 border border-green-500/20 flex items-center justify-center text-2xl">
                    {step.icon}
                  </div>
                  <span className="text-[10px] text-green-500/70 font-black tracking-wider">{step.num}</span>
                </div>

                {/* Connector (between steps on desktop) */}
                {i < 2 && (
                  <div className="hidden md:block step-connector mx-4 mt-7" />
                )}
                {/* Mobile vertical connector */}
                {i < 2 && (
                  <div className="md:hidden w-px h-8 bg-green-500/20 ml-7 mt-1 self-start" />
                )}
          ...
          <div className="text-center mt-12">
            <Link
              href="/auth/register"
              className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white px-8 py-3.5 rounded-full font-bold text-sm transition-all glow-green"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {FEATURES.map((f) => (
              <div key={f.title} className="glass-card rounded-2xl p-6 flex flex-col gap-4 card-hover-glow transition-all duration-300">
                <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${f.color} flex items-center justify-center text-xl`}>
                  {f.icon}
                </div>
      ...
      <section className="bg-alt-section py-20 px-4 border-t border-white/[0.05]">
        <div className="max-w-4xl mx-auto">
          <div className="glass-card rounded-3xl p-8 sm:p-12 text-center relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-br from-green-600/10 via-transparent to-cyan-600/5 rounded-3xl pointer-events-none" />
            <div className="relative z-10">
              ...
              <Link
                href="/about"
                className="inline-flex items-center gap-2 bg-white/[0.08] hover:bg-white/[0.13] border border-white/[0.12] hover:border-green-500/30 text-white px-6 py-2.5 rounded-full text-sm font-semibold transition-all"
              >
      ...
      <section className="max-w-7xl mx-auto px-4 py-20 border-t border-white/[0.05]">
        ...
        <div className="text-center mt-8">
          <Link
            href="/gallery"
            className="inline-flex items-center gap-2 bg-white/[0.05] hover:bg-white/[0.09] border border-white/[0.1] hover:border-green-500/30 text-white/65 hover:text-white px-6 py-2.5 rounded-full text-sm font-medium transition-all"
          >
