...
function ShareBar({ title, t }: {
  ...
  return (
    <div className="mt-8 bg-white rounded-2xl border border-slate-100 shadow-[0_1px_4px_rgba(15,23,42,0.06)] px-5 sm:px-7 py-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
      <div className="shrink-0">
        <p className="text-xs font-black text-slate-400 uppercase tracking-widest">
          {t('Share','シェア','শেয়ার')}
        </p>
        <p className="text-[11px] text-slate-300 mt-0.5">
          {t('Help others find this','シェアして広めよう','অন্যদের জানতে সাহায্য করুন')}
        </p>
      </div>
      <div className="flex items-center gap-2 flex-wrap">
        <button onClick={copyLink}
          className={`inline-flex items-center gap-1.5 h-9 px-4 rounded-xl text-xs font-bold transition-all duration-150 border
            ${copied
          ...
          {copied ? t('Copied!','コピー完了','কপি হয়েছে') : t('Copy link','リンクをコピー','লিংক কপি')}
        </button>

        <a href={`https://wa.me/?text=${encodedT}%20${encoded}`} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 h-9 px-4 rounded-xl text-xs font-bold border bg-[#f0fdf4] border-[#bbf7d0] text-[#15803d] hover:bg-[#dcfce7] transition-colors">
        ...
        <a href={`https://twitter.com/intent/tweet?text=${encodedT}&url=${encoded}`} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 h-9 px-4 rounded-xl text-xs font-bold border bg-slate-900 border-slate-800 text-white hover:bg-slate-800 transition-colors">
        ...
        <a href={`https://www.facebook.com/sharer/sharer.php?u=${encoded}`} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 h-9 px-4 rounded-xl text-xs font-bold border bg-[#eff6ff] border-[#bfdbfe] text-[#1d4ed8] hover:bg-[#dbeafe] transition-colors">
          <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
