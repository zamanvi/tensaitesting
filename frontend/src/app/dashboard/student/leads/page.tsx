...
export default function StudentApplicationPage() {
    ...
    <aside className="w-full md:w-72 shrink-0">
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="px-4 py-3.5 border-b border-slate-100 flex items-center gap-2">
          ...
          <span className="text-xs font-black text-slate-800 uppercase tracking-wide">Your Applications</span>
        </div>
        ...
        {isLoading ? (
          ...
          <button
            ...
            <div className="mt-3">
              ...
              <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${myApp.progress >= 80 ? 'bg-emerald-500' : myApp.progress >= 50 ? 'bg-amber-400' : 'bg-rose-400'}`}
          ...
          <div className="px-4 py-6 text-center">
            ...
            <p className="text-xs font-bold text-slate-400">No applications yet</p>
  ...
  // ── Main: Detail (submitted / accepted / rejected) ──────────────────────────
  const MainDetail = () => {
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-amber-500 to-amber-400 px-6 py-5">
              <p className="text-xs font-bold text-amber-100 uppercase tracking-wider mb-1">Ongoing</p>
                    ...
                    return (
                      <div key={step} className="flex flex-col items-center gap-2.5">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 z-10 ${
                          ...
                          active ? 'bg-white border-amber-400 shadow-sm' :
                        ...
                        <p className={`text-xs font-bold ${active ? 'text-amber-600' : done ? 'text-slate-500' : 'text-slate-300'}`}>{step}</p>
                      </div>
                    );
                  })}
                </div>
