...
export default function AffiliateSettingsPage() {
  ...
  useEffect(() => {
    if (user && user.gateway_type !== 'affiliate') router.replace(`/dashboard/${user.gateway_type}`);
  }, [user, router]);
  ...
  const currentAvatar = profileData?.profile?.avatar_url ?? user?.avatar_url ?? null;

  function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 2 * 1024 * 1024) { setAvatarErr(ja ? '2MB以下の画像を選択してください。' : bn ? '২ MB-এর নিচে ছবি বেছে নিন।' : 'Image must be under 2 MB.'); return; }
    setAvatarFile(file);
  ...
  const avatarMutation = useMutation({
    mutationFn: () => {
      ...
      return api.post('/affiliate/avatar', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
    },
    onSuccess: (res) => {
  ...
  const phoneMutation = useMutation({
    mutationFn: () => api.patch('/affiliate/settings', { phone }),
    onSuccess: () => {
  ...
  const pwMutation = useMutation({
    mutationFn: () => api.post('/affiliate/change-password', pwForm),
    onSuccess: () => {
    ...
    onError: (e: unknown) => {
      ...
      setPwErr(errs ? Object.values(errs).flat().join(' ') : err.response?.data?.message ?? (ja ? 'パスワード変更に失敗しました。' : bn ? 'পাসওয়ার্ড পরিবর্তন ব্যর্থ হয়েছে।' : 'Failed to change password.'));
    },
  });

  function setPw(k: keyof typeof pwForm, v: string) { setPwForm(f => ({ ...f, [k]: v })); }

  if (!user || user.gateway_type !== 'affiliate') return null;
      ...
      <div className="max-w-xl space-y-5">
            ...
            <div className="flex-1">
              ...
              {avatarFile && (
                <button
                  ...
                  className="mt-2 px-4 py-2 bg-indigo-700 hover:bg-indigo-800 text-white text-xs font-bold rounded-xl transition-colors disabled:opacity-50"
                >
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="shrink-0 px-4 py-2.5 bg-indigo-700 hover:bg-indigo-800 text-white text-xs font-bold rounded-xl transition-colors disabled:opacity-50"
            >
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">
                {ja ? '新しいパスワード（確認）' : bn ? 'পাসওয়ার্ড নিশ্চিত করুন' : 'Confirm New Password'}
              </label>
              <input
                type="password"
                className={inputCls}
          ...
          <button
            ...
            className="mt-4 w-full py-2.5 bg-indigo-700 hover:bg-indigo-800 text-white text-sm font-bold rounded-xl transition-colors disabled:opacity-50"
          >
