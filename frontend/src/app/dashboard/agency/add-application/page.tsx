...
export default function AgencyAddApplicationPage() {
  ...
  if (profileData && !approved) {
    return (
      <DashboardLayout title="Add Application">
        <div className="max-w-lg mx-auto mt-10 bg-amber-50 border border-amber-200 rounded-2xl p-8 text-center">
          ...
          <p className="text-sm text-slate-600 mb-4">Your agency profile must be approved before you can add applications.</p>
          <Link href="/dashboard/agency/profile" className="inline-block px-5 py-2.5 bg-green-700 hover:bg-green-800 text-white text-sm font-bold rounded-xl transition-colors">View Profile</Link>
        </div>
      </DashboardLayout>
    );
  }
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm mb-6 overflow-hidden">
            <div className="bg-gradient-to-r from-green-700 to-emerald-600 px-6 py-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-green-100 text-xs mt-1">Submit student applications to the admin portal</p>
                </div>
            ...
            {showCreate && (
              <div className="px-6 py-6 bg-slate-50/60 border-b border-slate-100">
                {createErr && <div className="mb-4 p-3 bg-rose-50 border border-rose-100 rounded-2xl text-xs text-rose-600">⚠️ {createErr}</div>}
                ...
                <button onClick={() => createMut.mutate()}
                  ...
                  className="mt-5 flex items-center gap-2 px-6 py-3 bg-green-700 hover:bg-green-800 text-white text-sm font-bold rounded-2xl disabled:opacity-50 transition-all">
              ...
              <div className="px-6 py-8 text-center">
                ...
                <p className="text-sm font-semibold text-slate-600">
                  {isLoading ? 'Loading…' : apps.length === 0
                    ...
                    : 'Select an application below to continue or click New Application'}
                </p>
              </div>
            )}
          </div>
