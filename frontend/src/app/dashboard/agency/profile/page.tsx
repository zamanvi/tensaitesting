...
export default function AgencyProfilePage() {
  ...
  const STATUS_BANNER: Record<string, { bg: string; icon: string; title: string; desc: string }> = {
    pending:      { bg: 'bg-amber-50 border-amber-200',   icon: '⏳', title: ja ? '審査待ち' : bn ? 'পর্যালোচনা অপেক্ষায়' : 'Under Review',    desc: ja ? '管理者がプロフィールを確認中です。通常24-48時間かかります。' : bn ? 'অ্যাডমিন আপনার প্রোফাইল যাচাই করছেন। সাধারণত ২৪-৪৮ ঘন্টা লাগে।' : 'Admin is reviewing your profile. Usually takes 24–48 hours.' },
    under_review: { bg: 'bg-blue-50 border-blue-200',     icon: '\u{1F50D}', title: ja ? '詳細審査中' : bn ? 'বিস্তারিত যাচাই চলছে' : 'In Detailed Review', desc: ja ? '追加確認が行われています。まもなく連絡があります。' : bn ? 'আরও যাচাই চলছে। শীঘ্রই যোগাযোগ করা হবে।' : 'Additional verification in progress. You will be contacted soon.' },
    approved:     { bg: 'bg-emerald-50 border-emerald-200', icon: '✅', title: ja ? '承認済み ✓' : bn ? 'অনুমোদিত ✓' : 'Approved ✓',      desc: ja ? `スロット番号 #${data?.slot_number} — フルアクセスが有効です。` : bn ? `স্লট নম্বর #${data?.slot_number} — সম্পূর্ণ অ্যাক্সেস সক্রিয়।` : `Slot #${data?.slot_number} — Full platform access active.` },
  ...
  return (
    <AgencyLayout title={ja ? 'エージェンシープロフィール' : bn ? 'এজেন্সি প্রোফাইল' : 'Agency Profile'}>
          ...
          <div>
            ...
            {status === 'rejected' && (
              <p className="text-xs text-red-600 font-medium mt-1">
                {ja ? '情報を修正して再提出できます。' : bn ? 'তথ্য সংশোধন করে পুনরায় জমা দিতে পারেন।' : 'You can correct the information and resubmit.'}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 mt-0.5">
              {ja ? 'プロフィールを提出することで、管理者の審査が始まります。承認後、プラットフォームへのフルアクセスが可能になります。' : bn ? 'প্রোফাইল জমা দিলে অ্যাডমিন যাচাই শুরু হবে। অনুমোদনের পর সম্পূর্ণ প্ল্যাটফর্ম অ্যাক্সেস পাবেন।' : 'Submit your profile to start admin vetting. Once approved, you get full platform access.'}
            </p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <input className={inputCls} placeholder="e.g. Global Study Consultancy" value={form.agency_name} onChange={e => set('agency_name', e.target.value)} required />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs font-semibold text-slate-500 mb-1 uppercase tracking-wide">
                {ja ? 'エージェンシー名 (ベンガル語)' : bn ? 'এজেন্সির নাম (বাংলা)' : 'Agency Name (Bangla)'}
              </label>
              <input className={inputCls} placeholder="বাংলায় নাম" value={form.agency_name_bn} onChange={e => set('agency_name_bn', e.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1 uppercase tracking-wide">
                {ja ? '登録番号' : bn ? 'রেজিস্ট্রেশন নম্বর' : 'Registration Number'}
              </label>
              <input className={inputCls} placeholder="e.g. REG-2024-XXXXX" value={form.registration_number} onChange={e => set('registration_number', e.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1 uppercase tracking-wide">
                {ja ? 'トレードライセンス番号' : bn ? 'ট্রেড লাইসেন্স নম্বর' : 'Trade License Number'}
              </label>
              <input className={inputCls} placeholder="e.g. TL-XXXXXXXX" value={form.trade_license} onChange={e => set('trade_license', e.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1 uppercase tracking-wide">
                {ja ? 'ウェブサイト' : bn ? 'ওয়েবসাইট' : 'Website'}
              </label>
              <input className={inputCls} type="url" placeholder="https://yourwebsite.com" value={form.website} onChange={e => set('website', e.target.value)} />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs font-semibold text-slate-500 mb-1 uppercase tracking-wide">
                {ja ? '説明' : bn ? 'বিবরণ' : 'Description'}
              </label>
              <textarea className={`${inputCls} resize-none`} rows={3} placeholder={ja ? 'エージェンシーについて説明してください...' : bn ? 'আপনার এজেন্সি সম্পর্কে বলুন...' : 'Tell us about your agency...'} value={form.description} onChange={e => set('description', e.target.value)} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input className={inputCls} placeholder="Full name" value={form.contact_person_name} onChange={e => set('contact_person_name', e.target.value)} required />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1 uppercase tracking-wide">
            ...
            <div className="sm:col-span-2">
              ...
              <input className={inputCls} placeholder="Street address, area" value={form.address} onChange={e => set('address', e.target.value)} required />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1 uppercase tracking-wide">
                {ja ? '市区町村' : bn ? 'শহর' : 'City'} <span className="text-red-400">*</span>
              </label>
              <input className={inputCls} placeholder="e.g. Dhaka" value={form.city} onChange={e => set('city', e.target.value)} required />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          <h3 className="font-bold text-slate-800 text-sm border-b border-slate-100 pb-3 mb-4">
            {ja ? 'サービス内容' : bn ? 'সেবাসমূহ' : 'Services Offered'}
          </h3>
          <div className="flex flex-wrap gap-2">
            {SERVICES.map(s => (
