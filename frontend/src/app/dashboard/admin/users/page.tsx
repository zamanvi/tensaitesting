    ...
    <DashboardLayout title={title}>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-5 space-y-3">
        <input
          ...
          className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500"
        />
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map(u => (
                  <tr key={u.id} className="hover:bg-green-50/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${TYPE_COLORS[u.gateway_type] ?? 'bg-slate-100 text-slate-600'}`}>
                        {u.gateway_type}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${STATUS_COLORS[u.status] ?? 'bg-slate-100 text-slate-600'}`}>
                        {u.status}
                      </span>
                    </td>
