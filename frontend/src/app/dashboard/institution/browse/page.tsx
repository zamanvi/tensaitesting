...
export default function BrowsePublishedLeads() {
  const { lang } = useLang();
  const ja = lang === 'ja';
  const bn = lang === 'bn';

  const [filters, setFilters] = useState({ jlpt_level: '', nat_level: '', min_gpa: '', gender: '' });
  const [applied, setApplied] = useState(filters);

  // Contact request modal state
  const [contactLead, setContactLead]   = useState<PublishedLead | null>(null);
  const [contactForm, setContactForm]   = useState<ContactForm>({ message: '', proposed_course: '', proposed_intake: '' });
  const [contactDone, setContactDone]   = useState<string>(''); // reference number on success
  const [contactError, setContactError] = useState('');

  // Track which leads we already sent a request for (session-level, prevents double-click)
  ...
  const leads: PublishedLead[] = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : [];

  function openContact(lead: PublishedLead) {
    setContactLead(lead);
    setContactForm({ message: '', proposed_course: '', proposed_intake: '' });
    setContactDone('');
    setContactError('');
  }
  ...
  return (
    <DashboardLayout title={ja ? '公開されたリード' : bn ? 'প্রকাশিত লিড' : 'Published Leads'}>
      ...
      <div className="mb-5 p-4 bg-blue-50 border border-blue-100 rounded-2xl text-sm text-blue-800 flex items-start gap-3">
        ...
        <div>
          <p className="font-semibold">
            {ja ? 'エージェンシーが公開した学生リード' : bn ? 'এজেন্সির প্রকাশিত শিক্ষার্থী লিড' : 'Agency-published student leads'}
          </p>
          <p className="text-xs text-blue-600 mt-0.5">
            {ja
              ? '興味のある学生に「コンタクト申請」を送ると、管理者が仲介します。'
              : bn
              ? 'পছন্দের শিক্ষার্থীতে "যোগাযোগের অনুরোধ" পাঠান — অ্যাডমিন মাধ্যমে যোগাযোগ হবে।'
              : 'Send a contact request for students you\'re interested in — admin will coordinate from there.'}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-4 mb-5">
        ...
        <div className="flex gap-2 mt-3">
          ...
          <button onClick={() => { setFilters({ jlpt_level: '', nat_level: '', min_gpa: '', gender: '' }); setApplied({ jlpt_level: '', nat_level: '', min_gpa: '', gender: '' }); }}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="font-semibold text-slate-600 mb-1">
            {ja ? '公開されたリードはありません' : bn ? 'কোনো প্রকাশিত লিড নেই' : 'No published leads yet'}
          </p>
          <p className="text-xs text-slate-400">
            {ja ? 'エージェンシーがリードを公開するとここに表示されます。' : bn ? 'এজেন্সি লিড প্রকাশ করলে এখানে দেখাবে।' : 'Leads will appear here once agencies publish them.'}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <span className="font-bold text-slate-900">{lead.student_name}</span>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                      ...
                      {lead.highest_qualification && <span>🎓 {lead.highest_qualification}</span>}
                      {lead.gpa        && <span>GPA {lead.gpa}</span>}
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400 mt-1.5">
                      ...
                      {lead.target_intake  && <span>📅 {lead.target_intake}</span>}
                      {lead.source_agency  && <span>🏢 {lead.source_agency}</span>}
                    </div>
                  </div>
                  ...
                  <div className="shrink-0">
                    {alreadySent ? (
                      ...
                      <button onClick={() => openContact(lead)}
                        ...
                        {ja ? '📩 コンタクト申請' : bn ? '📩 যোগাযোগের অনুরোধ' : '📩 Request Contact'}
                      </button>
                    )}
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="bg-slate-50 rounded-xl p-3 mb-4 text-sm">
                <div className="font-semibold text-slate-800">{contactLead.student_name}</div>
                <div className="text-xs text-slate-500 mt-0.5 font-mono">{contactLead.lead_code}</div>
              ...
              {contactDone ? (
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-xs text-slate-500 mb-1">
                    {ja ? '管理者が確認後、ご連絡します。' : bn ? 'অ্যাডমিন পর্যালোচনার পর আপনার সাথে যোগাযোগ করবে।' : 'Our team will review and follow up with you shortly.'}
                  </p>
                ...
                <form onSubmit={e => { e.preventDefault(); contactMutation.mutate(); }} className="space-y-3">
                  ...
                  <div>
                    ...
                    <textarea rows={4} required
                      ...
                      placeholder={ja
                        ? 'この学生に興味を持った理由、提供できるプログラムなど...'
                        : bn
                        ? 'এই শিক্ষার্থীতে আগ্রহী হওয়ার কারণ, প্রোগ্রামের বিবরণ...'
                        : 'Why you\'re interested in this student, what program you offer...'}
