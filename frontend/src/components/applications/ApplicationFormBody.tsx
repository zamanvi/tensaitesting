...
function ConfirmDialog({ message, detail, confirmLabel, onConfirm, onCancel, danger }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/40 backdrop-blur-sm" role="dialog" aria-modal="true">
      <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6">
        <h2 className="text-sm font-bold text-slate-900 mb-1">{message}</h2>
        ...
        <div className="flex gap-2.5">
          ...
          <button onClick={onConfirm}
            className={`flex-1 py-2.5 text-sm font-bold text-white rounded-xl transition-all ${danger ? 'bg-red-500 hover:bg-red-600' : 'bg-green-600 hover:bg-green-700'}`}>
            {confirmLabel}
          </button>
        </div>
      </div>
...
export default function ApplicationFormBody({
  ...
  async function handleSave() {
    ...
    try {
      ...
      const msg = ax.response?.status === 422 ? 'Validation error — check your inputs.'
        : ax.response?.data?.message ?? 'Save failed — please try again.';
      setErr(msg); setTimeout(() => setErr(''), 6000);
    }
    setSaving(false);
  }
      ...
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur border-b border-slate-100 px-4 sm:px-6 py-3 sm:py-4 shadow-sm">
        <div className="flex items-center justify-between gap-2 mb-2.5">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            {onClose && (
              <button onClick={handleClose} aria-label="Go back to applications list"
                className="w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-700 transition-colors flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-green-500/40">
            ...
            {app.status === 'submitted' && (
              <span className="text-xs font-bold px-2.5 py-1 bg-amber-100 text-amber-700 rounded-full flex-shrink-0">Submitted</span>
            )}
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            {canSubmit && (
              <button onClick={() => setConfirm('submit')} disabled={submitting} aria-label="Submit application"
        ...
        {msg && <p aria-live="polite" className="text-xs font-semibold text-green-600 mt-1.5 flex items-center gap-1">
        ...
        <div className="px-4 sm:px-6 py-6 sm:py-8 space-y-8 sm:space-y-10">
          ...
          {isEditable && (
            <div className="flex flex-col sm:flex-row gap-3 pt-2 pb-2">
              ...
              {canSubmit ? (
                <button onClick={() => setConfirm('submit')} disabled={submitting}
